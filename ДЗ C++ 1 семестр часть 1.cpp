...
private:
    ...
    static BigInt multiplication(const BigInt& , const BigInt& second_number);
...
BigInt BigInt::sum(BigInt first_number, BigInt second_number) {
    ...
    if (carry != 0)
        first_number.numbers.insert(first_number.numbers.begin() + 0, carry);
    return first_number;
}
...
BigInt BigInt::multiplication(const BigInt& first_number, const BigInt& second_number) {
    ...
    return BigInt::remove_leading_zeros(result);
}
...
BigInt BigInt::remainder_of_division(BigInt first_number, BigInt second_number) {
    ...
    return remove_leading_zeros(part_of_first);
}
