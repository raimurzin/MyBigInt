...
BigInt BigInt::take_merged_vectors(BigInt& first_number, BigInt& second_number) {
    if (first_number.sign == false) first_number.index_of_slash = first_number.numbers.size() + 1;
    else first_number.index_of_slash = first_number.numbers.size();
    for (int index_of_second = 0; index_of_second < second_number.numbers.size(); index_of_second++) first_number.numbers.push_back(second_number.numbers[index_of_second]);
    return first_number;
}
...
BigInt BigInt::get_numerator(BigInt number) {
    number.numbers.resize(number.index_of_slash);
    number.index_of_slash = 0;
    return number;
}
BigInt BigInt::get_denominator(BigInt number) {
    if (number.index_of_slash == 0) number.numbers = { 1 }; //Входящее число не является дробным
    do {
        number.numbers.erase(number.numbers.begin());
        number.index_of_slash--;
    } while (number.index_of_slash != 0);
    number.sign = true;
    return number;
}
...
char BigInt::compare(BigInt first_number, BigInt second_number) {
    ...
    if (first_number.index_of_slash != 0 || second_number.index_of_slash != 0) {
        BigInt first_numerator = get_numerator(first_number);
        BigInt first_denominator = get_denominator(first_number);
        BigInt second_numerator = get_numerator(second_number);
        BigInt second_denominator = get_denominator(second_number);
        return compare(first_numerator * second_denominator, second_numerator * first_denominator);
    }
...
BigInt BigInt::subtraction(BigInt first_number, BigInt second_number) {
    if (first_number.index_of_slash != 0 || second_number.index_of_slash != 0) {
        ...
        BigInt second_numerator = get_numerator(second_number);
        BigInt second_denominator = get_denominator(second_number);
        if (first_denominator == 0 || second_denominator == 0) throw "Error: incorrect fraction";
        if (first_denominator == 0 || second_denominator == 0) throw "Error: incorrect fraction";
        return ((first_numerator * second_denominator - second_numerator * first_denominator) / (first_denominator * second_denominator));
    }
...
BigInt BigInt::multiplication(const BigInt& first_number, const BigInt& second_number) {
    if (first_number.index_of_slash != 0 || second_number.index_of_slash != 0) {
        ...
        BigInt second_denominator = get_denominator(second_number);
        if (first_denominator == 0 || second_denominator == 0) throw "Error: incorrect fraction";
        return ((first_numerator * second_numerator) / (first_denominator * second_denominator));
    }

    if (first_number == 0 || second_number == 0) return 0;
    BigInt result;
    result.index_of_slash = 0;
...
BigInt BigInt::division(BigInt first_number, BigInt second_number) {
    if (first_number.index_of_slash != 0 || second_number.index_of_slash != 0) {
        BigInt first_numerator = get_numerator(first_number);
        BigInt first_denominator = get_denominator(first_number);
        BigInt second_numerator = get_numerator(second_number);
        BigInt second_denominator = get_denominator(second_number);
        if (first_denominator == 0 || second_denominator == 0) throw "Error: incorrect fraction";
        return ((first_numerator * second_denominator) / (first_denominator * second_numerator));
    }

    if (first_number % second_number != 0) {
        BigInt gcd = get_gcd(first_number, second_number);
        first_number = whole_of_division(first_number, gcd);
        second_number = whole_of_division(second_number, gcd);
        return take_merged_vectors(first_number, second_number);
    }
    else return whole_of_division(first_number, second_number);
}

std::ostream& operator <<(std::ostream& ostream, const BigInt& number) {
    std::string string = BigInt::convert_to_string(number);
    for (int i = 0; i < number.index_of_slash; i++) ostream.put(string[i]);
    if (number.index_of_slash != 0) ostream.put('/');
    for (int i = number.index_of_slash; i < string.size(); i++) ostream.put(string[i]);
    return ostream;
}
...
BigInt operator /(const BigInt& first_number, const BigInt& second_number) {
    return BigInt::division(first_number, second_number);
}
BigInt operator %(const BigInt& first_number, const BigInt& second_number) {
...
BigInt& BigInt::operator =(const std::string& string) {
    ...
    numbers = tmp.numbers;
    sign = tmp.sign;
    index_of_slash = tmp.index_of_slash;
    return *this;
}
